...
public class CaptureLayout extends FrameLayout {
    ...
    private ClickListener rightLongClickListener;   //右边按钮长按监听
    public static RecordManager recordManager = RecordManager.getInstance();
    WeakReference<RecordManager> recordManagerWeakReference = new WeakReference<>(recordManager);
    ...
    public CaptureLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        recordManager = recordManagerWeakReference.get();
...
//            }
//        });
//        iv_custom_right.setOnLongClickListener(new OnLongClickListener() {
//            @Override
//            public boolean onLongClick(View view) {
//                if (rightLongClickListener != null) {
//                    rightLongClickListener.onClick();
//                }
//                return false;
//            }
//        });
        RxView.clicks(iv_custom_right)
                ...
                .subscribe(new Consumer<Object>() {
                    ...
                    public void accept(Object o) {
                        switch (recordState) {
                            case 0:
                                recordManager.start();
                                setRightIconSrc(R.drawable.ic_camera);
                                recordState = 1;
                                break;
                            case 1:
                                recordManager.pause();
                                setRightIconSrc(R.drawable.ic_flash_off);
                                recordState = 2;
                                break;
                            case 2:
...
//                recordManager.pause();.
                        Toast.makeText(getContext(), recordState == 1 ? "开始录音" : recordState == 2 ? "暂停录音" : "继续录音", Toast.LENGTH_SHORT).show();
                    }
                });
        RxView.longClicks(iv_custom_right)
                .subscribe(new Consumer<Object>() {
                    @Override
                    public void accept(Object o) {
    ...
    public void setIconSrc(int iconLeft, int iconRight) {
        ...
        if (this.iconLeft != 0) {
            iv_custom_left.setImageResource(R.drawable.album_default);
            iv_custom_left.setVisibility(VISIBLE);
            btn_return.setVisibility(GONE);
        } else {
            iv_custom_left.setVisibility(GONE);
        ...
        if (this.iconRight != 0) {
            iv_custom_right.setImageResource(R.drawable.record_default);
            iv_custom_right.setVisibility(VISIBLE);
        } else {
            iv_custom_right.setVisibility(GONE);
        }
    ...
    public void setRightLongClickListener(ClickListener rightClickListener) {
        this.rightLongClickListener = rightClickListener;
    }
}
